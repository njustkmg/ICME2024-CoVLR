...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.LayerNorm = LayerNormML(config.hidden_size, eps=config.layer_norm_eps)
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, is_cross_attention):
        ...
        self.query = LinearML(config.hidden_size, self.all_head_size)
        if is_cross_attention:
            self.key = LinearML(config.encoder_width, self.all_head_size)
            self.value = LinearML(config.encoder_width, self.all_head_size)
        else:
            self.key = LinearML(config.hidden_size, self.all_head_size)
            self.value = LinearML(config.hidden_size, self.all_head_size)
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = LinearML(config.hidden_size, config.hidden_size)
        self.LayerNorm = LayerNormML(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = LinearML(config.hidden_size, config.intermediate_size)
...
class BertOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = LinearML(config.intermediate_size, config.hidden_size)
        self.LayerNorm = LayerNormML(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        # self.decoder = LinearML(config.hidden_size, config.vocab_size, bias=False)
        self.decoder = LinearML(config.hidden_size, config.vocab_size, bias=True)
        # print('--------------self.decoder',self.decoder)
        # print('--------------self.decoder', self.decoder.weight)
        # print('--------------self.decoder.weight.fast', self.decoder.weight.fast)
        # self.bias = nn.Parameter(torch.zeros(config.vocab_size))
        # self.bias_fast
        # Need a link between the two variables so that the bias is correctly resized with `resize_token_embeddings`
        # self.decoder.weight.fast = self.bias
...
class BertOnlyNSPHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.seq_relationship = LinearML(config.hidden_size, 2)
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (LinearML, nn.Embedding)):
            # Slightly different from the TF version which uses truncated_normal for initialization
            # cf https://github.com/pytorch/pytorch/pull/5617
            module.weight.data.normal_(mean=0.0, std=self.config.initializer_range)
        elif isinstance(module, LayerNormML):
            module.bias.data.zero_()
            module.weight.data.fill_(1.0)
        if isinstance(module, LinearML) and module.bias is not None:
            module.bias.data.zero_()
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = LinearML(config.hidden_size, config.num_labels)
...
class BertForMultipleChoice(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = LinearML(config.hidden_size, 1)
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = BertModel(config, add_pooling_layer=False)
        self.qa_outputs = LinearML(config.hidden_size, config.num_labels)
